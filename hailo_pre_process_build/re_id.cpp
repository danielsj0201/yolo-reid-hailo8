...
std::vector<HailoROIPtr> create_crops(std::shared_ptr<HailoMat> image, HailoROIPtr roi)
{
    ...
    {
        // Only process detections with label "person"
        if (std::string(PERSON_LABEL) == detection->get_label())
        {
            // Extract tracking ID
            HailoUniqueIDPtr tracking_id_ptr = get_tracking_id(detection);
            if (!tracking_id_ptr) continue;

            // Get bbox
            HailoBBox bbox = detection->get_bbox();

  
            HailoROIPtr crop_roi = std::make_shared<HailoROI>(bbox);


            HailoDetectionPtr copied_detection = std::make_shared<HailoDetection>(
                bbox,
                detection->get_class_id(),
                detection->get_label(),
                detection->get_confidence()
            );
            crop_roi->add_object(copied_detection);


            crop_roi->add_object(tracking_id_ptr);


            crop_rois.emplace_back(crop_roi);
        }
    }
